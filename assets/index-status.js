/* =========================================================
   HBCE INDEX STATUS — LOCAL PUBLIC-PROOF STATUS HELPER
   ...
   - Displays local proof references as local-only status
   ========================================================= */
...
(function () {
  ...
  function bindPinUI() {
    ...
    const pinButton = $("btnPin");
    const pinStatus = $("pinStatus");

    if (!pinButton) return;

    pinButton.addEventListener("click", () => {
      const value = clean(pinInput && pinInput.value).toLowerCase();

      if (!isHex64Plus(value)) {
        if (pinStatus) {
          pinStatus.textContent = "Invalid: local pack root must be a hexadecimal hash with at least 64 characters.";
        }
        return;
      }

      localStorage.setItem(KEY_LAST_ROOT, value);
      localStorage.setItem(KEY_LAST_ROOT_TS, String(Date.now()));

      if (pinStatus) {
        pinStatus.textContent = "Pinned locally. This is a local browser reference, not a public registry validation.";
      }
  ...
  function bindClearUI() {
    const clearButton = $("btnClearLocalProof");
    const pinStatus = $("pinStatus");

    if (!clearButton) return;

    clearButton.addEventListener("click", () => {
      localStorage.removeItem(KEY_LAST_ROOT);
      localStorage.removeItem(KEY_LAST_ROOT_TS);

      if (pinStatus) {
        pinStatus.textContent = "Local proof reference cleared.";
      }
  ...
  function main() {
    document.documentElement.setAttribute("data-hbce-index-status", "local-only");
