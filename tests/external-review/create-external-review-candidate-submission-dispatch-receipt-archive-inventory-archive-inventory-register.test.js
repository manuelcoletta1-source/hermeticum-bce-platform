...
function validInventory(overrides = {}) {
  const body = {
    ...
    generated_at: '2026-09-23T08:05:00Z',
    submission_dispatch_receipt_archive_inventory_archive_inventory_scope: inventoryScope(),
    ...
    submission_dispatch_receipt_archive_inventory_archive_inventory_sha256: hash('f'),
  ...
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_items = SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_COMPONENTS.map((component, index) => ({
  ...
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_item_count = SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_COMPONENTS.length;
...
function inventoryResult(overrides = {}) {
  return {
    state: SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_STATE.INVENTORIED,
    reason: 'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_INVENTORIED',
    submission_dispatch_receipt_archive_inventory_archive_inventory: validInventory(overrides)
  };
}
...
function context() {
  return {
    ...
    generated_at: '2026-09-23T08:25:00Z',
...
{
  const actual = createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryRegister(inventoryResult(), context());
  const body = actual.submission_dispatch_receipt_archive_inventory_archive_inventory_register;

  assert.equal(actual.state, SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_STATE.REGISTERED);
  assert.equal(actual.reason, 'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_REGISTERED');
  assert.equal(body.proto, SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_PROTO);
  assert.equal(body.kind, SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_KIND);
  assert.match(body.submission_dispatch_receipt_archive_inventory_archive_inventory_register_sha256, /^[a-f0-9]{64}$/);
  assert.equal(body.submission_dispatch_receipt_archive_inventory_archive_inventory_register_components_satisfied, true);
  assert.equal(body.submission_dispatch_receipt_archive_inventory_archive_inventory_items_preserved, true);
  assert.equal(body.submission_dispatch_receipt_archive_inventory_archive_inventory_register_boundary_state, 'PRESERVED');
  assert.equal(body.public_registry_mutated, false);
  assert.equal(body.external_trust_registry_mutated, false);
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_REGISTERED_FROM_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY');
}

{
  const actual = createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryRegister(validInventory(), context());
  assert.equal(actual.state, SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_STATE.REGISTERED);
  assert.equal(actual.submission_dispatch_receipt_archive_inventory_archive_inventory_register.submission_dispatch_receipt_archive_inventory_archive_inventory_register_boundary_state, 'PRESERVED');
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_REGISTERED_FROM_INVENTORY_BODY');
}

{
  const actual = createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryRegister(
    inventoryResult({
  ...
  assert.equal(actual.state, SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_STATE.REGISTERED);
  assert.equal(actual.submission_dispatch_receipt_archive_inventory_archive_inventory_register.validation_reason, 'RPECRC_HUMAN_CONTROL_PENDING');
  assert.equal(actual.submission_dispatch_receipt_archive_inventory_archive_inventory_register.submission_dispatch_receipt_archive_inventory_archive_inventory_register_boundary_state, 'PRESERVED');
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_HUMAN_PENDING_REGISTERED');
}

{
  ...
  assert.equal(actual.state, SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_STATE.REGISTERED);
  assert.equal(actual.submission_dispatch_receipt_archive_inventory_archive_inventory_register.submission_dispatch_receipt_archive_inventory_archive_inventory_register_boundary_state, 'BLOCKED');
  assert.equal(actual.submission_dispatch_receipt_archive_inventory_archive_inventory_register.external_submission_dispatched, false);
  assert.equal(actual.submission_dispatch_receipt_archive_inventory_archive_inventory_register.external_archive_storage_created, false);
  assert.equal(actual.submission_dispatch_receipt_archive_inventory_archive_inventory_register.public_registry_mutated, false);
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_BOUNDARY_BLOCKED_REGISTERED');
}

{
  const actual = createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryRegister(inventoryResult(), {
    generated_at: '2026-09-23T08:25:00Z'
  });

  assert.deepEqual(actual, {
    state: SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_STATE.INVALID,
    reason: 'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_ID_INVALID'
  });
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_ID_INVALID');
}

{
  ...
  assert.deepEqual(actual, {
    state: SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_STATE.INVALID,
    reason: 'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_GENERATED_AT_INVALID'
  });
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_GENERATED_AT_INVALID');
}

{
  const actual = createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryRegister({ state: 'INVALID' }, context());

  assert.deepEqual(actual, {
    state: SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_STATE.INVALID,
    reason: 'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_INVALID'
  });
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_INVALID');
}

{
  ...
  assert.equal(body.submission_dispatch_receipt_archive_inventory_archive_inventory_register_component_count, SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_COMPONENTS.length);
  assert.equal(body.submission_dispatch_receipt_archive_inventory_archive_inventory_register_record_count, SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_COMPONENTS.length);
  assert.deepEqual(
    body.submission_dispatch_receipt_archive_inventory_archive_inventory_register_records.map(record => record.source_ref_field),
    SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_COMPONENTS.map(component => component.ref_field)
  );
  assert.equal(SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_COMPONENTS.length, 30);
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_COMPONENTS');
}

{
  const actual = createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryRegister(inventoryResult(), context());
  const scope = actual.submission_dispatch_receipt_archive_inventory_archive_inventory_register.submission_dispatch_receipt_archive_inventory_archive_inventory_register_scope;

  assert.equal(scope.INTERNAL_RECEIPT_CHAIN_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER, true);
  ...
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_SCOPE_FALSE');
}

{
  ...
  console.log('PASS RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_RUNTIME_EFFECTS_FALSE');
}
