...
} = require('./create-external-review-candidate-submission-dispatch-receipt-archive-inventory-archive-inventory-register');

const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_STATE = Object.freeze({
  LEDGERED: 'LEDGERED',
  INVALID: 'INVALID'
});

const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_PROTO = 'HBCE-RPECRC-INTERNAL-SUBMISSION-DISPATCH-RECEIPT-ARCHIVE-INVENTORY-ARCHIVE-INVENTORY-LEDGER-v1';
const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_KIND = 'PRODUCTION_READINESS_EXECUTION_CONTROL_EXTERNAL_REVIEW_CANDIDATE_INTERNAL_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER';

const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_SCOPE = Object.freeze({
...
const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_COMPONENTS = Object.freeze([
  Object.freeze({
    ledger_component_id: 'HBCE-RPECRC-SUBMISSION-DISPATCH-RECEIPT-ARCHIVE-INVENTORY-ARCHIVE-INVENTORY-LEDGER-COMPONENT-001',
    ledger_component_name: 'Internal Submission Dispatch Receipt Archive Inventory Archive Inventory Register',
    ref_field: 'submission_dispatch_receipt_archive_inventory_archive_inventory_register_sha256'
  }),
  ...SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_COMPONENTS.map((component, index) => Object.freeze({
    ledger_component_id: `HBCE-RPECRC-SUBMISSION-DISPATCH-RECEIPT-ARCHIVE-INVENTORY-ARCHIVE-INVENTORY-LEDGER-COMPONENT-${String(index + 2).padStart(3, '0')}`,
...
function result(state, reason, body) {
  if (body) {
    return Object.freeze({
      state,
      reason,
      submission_dispatch_receipt_archive_inventory_archive_inventory_ledger: body
    });
  }
...
function isRegisterBody(value) {
  return (
    isPlainObject(value) &&
    value.proto === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_PROTO &&
    value.kind === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_KIND &&
    isNonEmptyString(value.submission_dispatch_receipt_archive_inventory_archive_inventory_register_id) &&
    isNonEmptyString(value.submission_dispatch_receipt_archive_inventory_archive_inventory_register_sha256)
  );
}
...
function resolveRegister(source, context) {
  if (
    ...
    source.state === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_STATE.REGISTERED &&
    isRegisterBody(source.submission_dispatch_receipt_archive_inventory_archive_inventory_register)
  ) {
    return source.submission_dispatch_receipt_archive_inventory_archive_inventory_register;
  }
  ...
  const registerResult = createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryRegister(
    source,
    isPlainObject(context.submission_dispatch_receipt_archive_inventory_archive_inventory_register_context)
      ? context.submission_dispatch_receipt_archive_inventory_archive_inventory_register_context
      : {}
  );

  if (
    registerResult.state !== SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_STATE.REGISTERED ||
    !isRegisterBody(registerResult.submission_dispatch_receipt_archive_inventory_archive_inventory_register)
  ) {
    return null;
  }

  return registerResult.submission_dispatch_receipt_archive_inventory_archive_inventory_register;
}
...
function registerRecordsPreserved(register) {
  return (
    Array.isArray(register.submission_dispatch_receipt_archive_inventory_archive_inventory_register_records) &&
    register.submission_dispatch_receipt_archive_inventory_archive_inventory_register_record_count === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_COMPONENTS.length &&
    register.submission_dispatch_receipt_archive_inventory_archive_inventory_register_records.length === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_COMPONENTS.length
  );
}

function scopePreserved(register) {
  const scope = register.submission_dispatch_receipt_archive_inventory_archive_inventory_register_scope;

  return (
    isPlainObject(scope) &&
    scope.INTERNAL_TECHNICAL_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_ONLY === true &&
    scope.INTERNAL_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER === true &&
    scope.INTERNAL_RECEIPT_CHAIN_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER === true &&
...
function createLedgerEntries(register) {
  return SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_COMPONENTS.map((component, index) => Object.freeze({
...
function createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryLedger(source, context = {}) {
  if (!isPlainObject(context) || !isNonEmptyString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_id)) {
    return result(
      SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_STATE.INVALID,
      'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_ID_INVALID'
    );
  }
  ...
  if (!isIsoDateTime(context.generated_at)) {
    return result(
      SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_STATE.INVALID,
      'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_GENERATED_AT_INVALID'
    );
  }
  ...
  if (!register) {
    return result(
      SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_STATE.INVALID,
      'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REGISTER_INVALID'
    );
  }
  ...
  const preservedBoundary = (
    ...
    runtimeOk &&
    satisfiedOk &&
    register.submission_dispatch_receipt_archive_inventory_archive_inventory_register_boundary_state === 'PRESERVED'
  );
  ...
  const body = {
    ...
    submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_id: context.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_id,
    submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_version: defaultString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_version, '1.0'),
    ...
    submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_scope: SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_SCOPE,
    submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_ref: defaultString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_ref, 'internal-submission-dispatch-receipt-archive-inventory-archive-inventory-ledger:pending'),
    submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_method: defaultString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_method, 'INTERNAL_TECHNICAL_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_ONLY'),
    submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_material_state: defaultString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_material_state, 'INTERNAL_LEDGERED_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REFERENCES_ONLY')
  };

  copyRegisterFields(body, register);

  body.submission_dispatch_receipt_archive_inventory_archive_inventory_register_proto = register.proto;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_register_kind = register.kind;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_components = SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_COMPONENTS;
  ...
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_entries = entries;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_entry_count = entries.length;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_components_satisfied = componentsOk;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_register_records_preserved = recordsOk;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_boundary_state = preservedBoundary ? 'PRESERVED' : 'BLOCKED';

  for (const field of FALSE_RUNTIME_FIELDS) {
    body[field] = false;
  }

  body.submission_dispatch_receipt_archive_inventory_archive_inventory_ledger_sha256 = sha256Canonical(body);

  return result(
    SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_STATE.LEDGERED,
    'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_LEDGERED',
    deepFreeze(body)
  );
}
...
module.exports = Object.freeze({
  ...
  SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_LEDGER_SCOPE,
