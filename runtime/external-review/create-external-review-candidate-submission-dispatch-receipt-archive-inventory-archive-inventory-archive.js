...
} = require('./create-external-review-candidate-submission-dispatch-receipt-archive-inventory-archive-inventory-closure');

const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_STATE = Object.freeze({
  ARCHIVED: 'ARCHIVED',
  INVALID: 'INVALID'
});

const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_PROTO = 'HBCE-RPECRC-INTERNAL-SUBMISSION-DISPATCH-RECEIPT-ARCHIVE-INVENTORY-ARCHIVE-INVENTORY-ARCHIVE-v1';
const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_KIND = 'PRODUCTION_READINESS_EXECUTION_CONTROL_EXTERNAL_REVIEW_CANDIDATE_INTERNAL_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE';

const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_SCOPE = Object.freeze({
...
const SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_COMPONENTS = Object.freeze([
  Object.freeze({
    archive_component_id: 'HBCE-RPECRC-SUBMISSION-DISPATCH-RECEIPT-ARCHIVE-INVENTORY-ARCHIVE-INVENTORY-ARCHIVE-COMPONENT-001',
    archive_component_name: 'Internal Submission Dispatch Receipt Archive Inventory Archive Inventory Closure',
    ref_field: 'submission_dispatch_receipt_archive_inventory_archive_inventory_closure_sha256'
  }),
  ...SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE_COMPONENTS.map((component, index) => Object.freeze({
    archive_component_id: `HBCE-RPECRC-SUBMISSION-DISPATCH-RECEIPT-ARCHIVE-INVENTORY-ARCHIVE-INVENTORY-ARCHIVE-COMPONENT-${String(index + 2).padStart(3, '0')}`,
    archive_component_name: component.closure_component_name,
...
function result(state, reason, body) {
  if (body) {
    return Object.freeze({
      state,
      reason,
      submission_dispatch_receipt_archive_inventory_archive_inventory_archive: body
    });
  }
...
function isClosureBody(value) {
  return (
    isPlainObject(value) &&
    value.proto === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE_PROTO &&
    value.kind === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE_KIND &&
    isNonEmptyString(value.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_id) &&
    isNonEmptyString(value.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_sha256)
  );
}
...
function resolveClosure(source, context) {
  if (
    ...
    source.state === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE_STATE.CLOSED &&
    isClosureBody(source.submission_dispatch_receipt_archive_inventory_archive_inventory_closure)
  ) {
    return source.submission_dispatch_receipt_archive_inventory_archive_inventory_closure;
  }
  ...
  const closureResult = createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryClosure(
    source,
    isPlainObject(context.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_context)
      ? context.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_context
      : {}
  );

  if (
    closureResult.state !== SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE_STATE.CLOSED ||
    !isClosureBody(closureResult.submission_dispatch_receipt_archive_inventory_archive_inventory_closure)
  ) {
    return null;
  }

  return closureResult.submission_dispatch_receipt_archive_inventory_archive_inventory_closure;
}
...
function closureRecordsPreserved(closure) {
  return (
    Array.isArray(closure.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_records) &&
    closure.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_record_count === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE_COMPONENTS.length &&
    closure.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_records.length === SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE_COMPONENTS.length
  );
}

function scopePreserved(closure) {
  const scope = closure.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_scope;

  return (
    isPlainObject(scope) &&
    scope.INTERNAL_TECHNICAL_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE_ONLY === true &&
    scope.INTERNAL_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE === true &&
...
function createArchiveRecords(closure) {
  return SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_COMPONENTS.map((component, index) => Object.freeze({
    ...
    archive_component_ref: component.archive_component_id,
...
function createExternalReviewCandidateSubmissionDispatchReceiptArchiveInventoryArchiveInventoryArchive(source, context = {}) {
  if (!isPlainObject(context) || !isNonEmptyString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_id)) {
    return result(
      SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_STATE.INVALID,
      'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_ID_INVALID'
    );
  }
  ...
  if (!isIsoDateTime(context.generated_at)) {
    return result(
      SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_STATE.INVALID,
      'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_GENERATED_AT_INVALID'
    );
  }
  ...
  if (!closure) {
    return result(
      SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_STATE.INVALID,
      'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_CLOSURE_INVALID'
    );
  }
  ...
  const preservedBoundary = (
    ...
    runtimeOk &&
    satisfiedOk &&
    closure.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_boundary_state === 'PRESERVED'
  );

  const archiveRecords = createArchiveRecords(closure);
  ...
  const body = {
    ...
    submission_dispatch_receipt_archive_inventory_archive_inventory_archive_id: context.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_id,
    submission_dispatch_receipt_archive_inventory_archive_inventory_archive_version: defaultString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_version, '1.0'),
    ...
    submission_dispatch_receipt_archive_inventory_archive_inventory_archive_scope: SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_SCOPE,
    submission_dispatch_receipt_archive_inventory_archive_inventory_archive_ref: defaultString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_ref, 'internal-submission-dispatch-receipt-archive-inventory-archive-inventory-archive:pending'),
    submission_dispatch_receipt_archive_inventory_archive_inventory_archive_method: defaultString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_method, 'INTERNAL_TECHNICAL_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_ONLY'),
    submission_dispatch_receipt_archive_inventory_archive_inventory_archive_material_state: defaultString(context.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_material_state, 'INTERNAL_ARCHIVED_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_REFERENCES_ONLY')
  };

  copyClosureFields(body, closure);

  body.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_proto = closure.proto;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_kind = closure.kind;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_components = SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_COMPONENTS;
  ...
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_records = archiveRecords;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_record_count = archiveRecords.length;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_components_satisfied = componentsOk;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_closure_records_preserved = recordsOk;
  body.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_boundary_state = preservedBoundary ? 'PRESERVED' : 'BLOCKED';

  for (const field of FALSE_RUNTIME_FIELDS) {
    body[field] = false;
  }

  body.submission_dispatch_receipt_archive_inventory_archive_inventory_archive_sha256 = sha256Canonical(body);

  return result(
    SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_STATE.ARCHIVED,
    'RPECRC_SUBMISSION_DISPATCH_RECEIPT_ARCHIVE_INVENTORY_ARCHIVE_INVENTORY_ARCHIVE_ARCHIVED',
    deepFreeze(body)
  );
}
